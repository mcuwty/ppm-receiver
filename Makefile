...
#自定义源文件添加
C_SRC			+= $(HARD_DIR)/usart.c $(HARD_DIR)/spi.c $(HARD_DIR)/delay.c $(HARD_DIR)/led.c
C_SRC			+= $(APP)/nrf24l01.c $(APP)/ppm.c $(APP)/sbus.c
...
#openocd相关选项参数
OOCD			= openocd
OOCDFLAGS		= -f /usr/local/share/openocd/scripts/interface/stlink-v2.cfg  -f /usr/local/share/openocd/scripts/target/stm32f1x.cfg
#OOCDFLAGS       = -f /usr/local/share/openocd/scripts/interface/jlink.cfg  -f /usr/local/share/openocd/scripts/target/stm32f1x.cfg

#伪目标
.PHONY: all clean rebuild debug upload test ctags
